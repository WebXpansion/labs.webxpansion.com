...
// One-time splash screen shown before the site is revealed — adapted from a
// CodePen "text crack" animation. Uses sessionStorage so it plays once per
// browser session (first load / hard refresh), not on every in-app
// navigation.
//
// The heavy part of the site (the WebGL/Three.js slider) is NOT mounted
// behind this overlay while it plays: on a slower machine, mounting that
// scene at the same time as the loader animation competes for the main
// thread and can delay the setTimeout calls below by several seconds,
// which looks like the loading screen is "stuck". Instead the parent
// (App.tsx) waits for `onReveal` before mounting the rest of the app, and
// we call it as soon as we start fading out — that gives the real site a
// head start to initialise while it's still hidden behind the fade.
const STORAGE_KEY = 'wxl-loader-seen'

// Timings mirror the original CSS: title scale/blur/quiver/shake finish
// around 3s, the crack effect fires at 3s + 100ms. We hold briefly after
// that, then fade the whole overlay out.
const CRACK_END_MS = 3100
const HOLD_MS = 350
const FADE_MS = 600
...
export function Loader({ onReveal }: LoaderProps) {
  ...
  // In dev, React's StrictMode runs this effect twice (mount → cleanup →
  // mount again) to help catch impure effects. That used to break this
  // component: the first run would write STORAGE_KEY to sessionStorage and
  // schedule the timers, StrictMode's cleanup would clear those timers, and
  // the second run would then read the flag it had *just* written, think
  // "already seen", and skip scheduling new ones — leaving the overlay
  // stuck on 'visible' forever with nothing left to move it to 'fading'.
  // This ref records the decision once per component instance (it survives
  // the double effect run, unlike a local variable) so the sessionStorage
  // read/write only ever happens on the first run, while the timers still
  // get (re)scheduled on whichever run is the one that actually sticks.
  const decisionRef = useRef<'pending' | 'show' | 'skip'>('pending')
  ...
  useEffect(() => {
    ...
    const fadeTimer = setTimeout(() => {
      setPhase('fading')
      // Start mounting the real site now, while it's still hidden behind
      // the (compositor-driven) fade-out — by the time opacity reaches 0
      // it should already be initialised.
      onRevealRef.current?.()
    }, CRACK_END_MS + HOLD_MS)
    const hideTimer = setTimeout(() => setPhase('hidden'), CRACK_END_MS + HOLD_MS + FADE_MS)
    return () => {
  ...
  return (
    ...
    >
      ...
      >
  <defs>
    <text id="loader-title" fill="#bfbfc8" fontSize="60" fontWeight="bold" letterSpacing="2" fontFamily="Familjen Grotesk" textAnchor="middle">
      WebXpansion Labs
    </text>
    <filter id="loader-blur" width="1.0112729" height="1.1185185" x="-0.0056364482" y="-0.059259259" colorInterpolationFilters="sRGB">
      <feGaussianBlur id="loader-gaussian-blur" stdDeviation="1 1" result="blur" />
    </filter>
  </defs>
  <g id="loader-title-text" filter="url(#loader-blur)">
    {/* x=400 is the exact horizontal center of the 0-800 viewBox; using
        text-anchor="middle" here (rather than the original hardcoded
        x=195.3 + default "start" anchor) keeps the title centered no
        matter which font actually ends up rendering it. */}
    <use id="loader-title-left" x="400" y="421.1" xlinkHref="#loader-title" />
    <use id="loader-title-middle" x="400" y="421.1" xlinkHref="#loader-title" />
    <use id="loader-title-right" x="400" y="421.1" xlinkHref="#loader-title" />
  </g>
  <g id="loader-crack" opacity="0">
    <path fill="none" stroke="#f2f2f2" strokeWidth="0.0967312" d="m 754.07875,539.71842 c -3.69792,-1.31368 -15.15169,-6.14702 -21.19332,-8.79913" />
    <path fill="none" stroke="#ffffff" strokeWidth="0.18" d="m 770.76967,547.26354 -16.59358,-7.47881" />
    <path fill="none" stroke="#f2f2f2" strokeWidth="0.1" d="m 801.03572,561.39752 c -3.55723,-1.88939 -25.10819,-11.22235 -30.29136,-14.16331" />
    ...
    <path fill="none" strokeWidth="0.1" d="m 691.4264,517.89975 c -3.55723,-1.88939 -17.25763,-9.75744 -22.4408,-12.6984" />
    <path fill="none" strokeWidth="0.18" d="m 707.10399,527.33945 c -3.4553,-2.13201 -10.9408,-6.65111 -16.12397,-9.59207" />
    <path fill="none" strokeWidth="0.1" d="m 735.82031,545.8878 c -3.55723,-1.88939 -23.54422,-15.59643 -28.72739,-18.53739" />
    <path fill="none" strokeWidth="0.2" d="m 765.45665,562.19213 c -4.98529,-1.67302 -24.45299,-13.25961 -29.63616,-16.20057" />
    ...
    <path fill="#292c2e" fillOpacity="0.901314" d="m 396.85237,416.49325 c 0,0.17677 1.27373,0.96226 1.27373,0.96226 l 0.7745,0.0566 -0.68301,1.09335 1.07054,-0.9056 1.49793,0.71 c 2.87673,-0.10842 4.09136,-0.0716 3.41577,-0.23919 -0.98755,-0.24503 -4.91676,-0.92928 -5.30765,-1.16436 z" />
    ...
    <path fill="#8a8a9b" d="m 434.8271,401.16961 4.42619,1.89036 c 0,0 -1.24486,0 -1.42929,-0.0922 -0.18442,-0.0922 -3.31964,-1.84424 -3.31964,-1.84424 z" />
      ...
      <style>{`
        ...
        :root {
          --loader-crack-delay: 3s;
          --loader-crack-duration: 100ms;
        }
        #loader-title-text {
          animation-name: loader-scale, loader-blur, loader-quiver, loader-shake;
        ...
        #loader-title-left {
          ...
          animation-name: loader-crack-left;
          animation-delay: var(--loader-crack-delay);
          animation-duration: var(--loader-crack-duration);
        ...
        #loader-title-right {
          ...
          animation-name: loader-crack-right;
          animation-delay: var(--loader-crack-delay);
          animation-duration: var(--loader-crack-duration);
          animation-fill-mode: forwards;
        }
        ...
        @keyframes loader-crack {
          to {
            clip-path: var(--loader-crack-clip);
          }
        }
        @keyframes loader-crack-right {
          to {
            clip-path: var(--loader-crack-clip);
            translate: 1% -1%;
          }
        }
        @keyframes loader-crack-left {
          to {
            clip-path: var(--loader-crack-clip);
            translate: -0.45% 0.35%;
          }
